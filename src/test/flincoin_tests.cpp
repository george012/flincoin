...
BOOST_AUTO_TEST_CASE(subsidy_100k_145k_test)
{
    ...
    for (int nHeight = 100000; nHeight <= 1000; nHeight++) {
        const Consensus::Params& params = mainParams.GetConsensus(nHeight);
        CAmount nSubsidy = GetFlincoinBlockSubsidy(nHeight, params, ArithToUint256(prevHash));
        BOOST_CHECK(MoneyRange(nSubsidy));
...
BOOST_AUTO_TEST_CASE(subsidy_post_145k_test)
{
    ...
    for (int nHeight = 1000; nHeight < 600000; nHeight++) {
        const Consensus::Params& params = mainParams.GetConsensus(nHeight);
...
BOOST_AUTO_TEST_CASE(hardfork_parameters)
{
    ...
    const Consensus::Params& digishieldParams = Params().GetConsensus(1000);
    ...
    const Consensus::Params& digishieldParamsEnd = Params().GetConsensus(6999);
    ...
    const Consensus::Params& auxpowParams = Params().GetConsensus(7000);
    BOOST_CHECK_EQUAL(auxpowParams.nHeightEffective, 7000);
