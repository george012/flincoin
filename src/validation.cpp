...
#if defined(NDEBUG)
# error "Flincoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Flincoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetFlincoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Flincoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("flincoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Flincoin: BIP30 has been active since inception
    ...
    // Flincoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetFlincoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Flincoin: Disallow AuxPow blocks before it is activated.
    ...
    // Flincoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Flincoin: We probably want to disable this
    int nLockTimeFlags = 0;
