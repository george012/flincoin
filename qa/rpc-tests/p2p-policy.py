...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        ### test constants ###
        flins = Decimal("0.00000001")          # 1 Koinu expressed in FLIN
        ...
        output = { self.tgtAddr: ten - relay_fee_per_byte * 192 + flins }
        ...
        amount = soft_dust_limit - flins
        change = ten - amount - relay_fee_per_byte * 225 - soft_dust_limit + flins
        output = { self.tgtAddr : amount, self.srcAddr: change }
        self.run_relay_test(output, 66, 225)

        # test a 1 flins output with sufficient fee
        amount = flins
        change = ten - amount - relay_fee_per_byte * 225 - soft_dust_limit
        output = { self.tgtAddr : amount, self.srcAddr: change }
        self.run_relay_test(output, 64, 225) # 64 = dust

        # test a 1 flins output with insufficient fee
        amount = flins
        change = ten - amount - relay_fee_per_byte * 225 - soft_dust_limit + flins
        output = { self.tgtAddr : amount, self.srcAddr: change }
