Flincoin Core version 1.14.8 is now available from:

  <https://github.com/flincoin/flincoin/releases/tag/v1.14.8/>

This is a new minor version release. Flincoin users on any platform are
...
Several important updates have been added to Flincoin Core, including fixes
backported from Bitcoin Core's upstream.

### Reduce Peer INV Object Counts

Previously, Flincoin Core nodes would maintain a large list of inventory for
...
A new transaction request tracking system improves the way Flincoin Core
...
In previous versions of Flincoin Core, orphan transactions were processed in a
...
Flincoin Core now sends a getheaders message for one block when announced in an
...
vulnerability or security issue in Flincoin Core.
...
* Flincoin Core now supports compilation with C++14 and C++17, if you select
